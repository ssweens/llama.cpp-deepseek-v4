...
// swiglu_clamped: gate = clamp(gate, -INF, +limit), up = clamp(up, -limit, +limit), out = silu(gate) * up
// (DeepSeek-V3/V4 / Step3.5 SwiGLU clamping path; matches the manual CLAMP+CLAMP+SILU+MUL chain.)

template <typename T>
static __global__ void swiglu_clamped_kernel(const T * x, const T * g, T * dst,
    ...
    float gate = (float) x[j0];
    float up   = (float) g[j1];

    // clamp(gate, -INF, +limit)
...
template <typename T>
static void swiglu_clamped_cuda(const T * x, const T * g, T * dst,
                                const int64_t k, const int64_t n,
                                const int64_t o0, const int64_t o1,
                                const float limit, cudaStream_t stream) {
    const int64_t num_blocks = (k + CUDA_GLU_BLOCK_SIZE - 1) / CUDA_GLU_BLOCK_SIZE;
    swiglu_clamped_kernel<<<num_blocks, CUDA_GLU_BLOCK_SIZE, 0, stream>>>(x, g, dst, k, n, o0, o1, limit);
}

void ggml_cuda_op_swiglu_clamped(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    const ggml_tensor * src0 = dst->src[0];
    const ggml_tensor * src1 = dst->src[1];
    void * src0_d = src0->data;
    void * src1_d = src1 ? src1->data : src0->data;
    ...
    swiglu_clamped_cuda(src0_p, src1_p, (float *) dst_d, ggml_nelements(dst), nc,
                        src0_o / sizeof(float), src1_o / sizeof(float), limit, stream);
}
