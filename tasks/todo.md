...
- [x] **`GGML_GLU_OP_SWIGLU_CLAMPED` (commit `289fcb280`)**: new GLU variant for DSv4/Step3.5 swiglu clamping. Eliminates a 4-op (CLAMP+CLAMP+SILU+MUL) chain in `build_moe_ffn`. Decode +1.3% (56.4 -> 57.1 t/s). Fusion-args plumbing for `glu_limit` left for a future change; without it, the existing `MUL_MAT_ID + MUL_MAT_ID + GLU` fusion is still rejected for SWIGLU_CLAMPED (would silently miscompute due to a latent default-param bug in mmvq/mmq fused kernels).
- [ ] **Plumb `glu_limit` through `ggml_cuda_mm_fusion_args_*`**: lets MUL_MAT_ID + MUL_MAT_ID + GLU fusion fire for SWIGLU_CLAMPED (DSv4), and incidentally fixes the latent SWIGLU_OAI default-param issue for any model using non-default alpha/limit. ~3-4h of work, expected gain another ~3-5% TPS.
