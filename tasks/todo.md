...
- [x] **CONT reduction (commit `7013be05c`)**: replaced 34 `ggml_cont()` calls in `deepseek4.cpp` with a graph-build-time `cont_if_needed()` helper that skips the materialization when the input is already contiguous. CONT calls dropped from 17362 to 1602 (90% reduction). Total measured compute went from 1550 ms to 1426 ms (-8%). Long-prompt decode +14% vs original baseline; prefill recovered to roughly equal.
- [ ] **Future perf-optimization candidates** (next, in priority order):
  ...
  3. `CPY` (6.9%) and `SET_ROWS` (3.9%) reductions: same idea as CONT; check whether some KV cache writes can be elided via in-place views.
