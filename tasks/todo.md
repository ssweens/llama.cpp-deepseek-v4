...
- Tested and rejected exposing per-row verifier raw top-1 values to continue drafting after partial accepts. It reduced stale-gate skips in trace, but made MTP-enabled tool replay fail again and produced unstable draft-max-2 performance (`tg32 30.51 ± 2.61`). Reverted the experiment. Artifacts: `/tmp/dsv4_iq2xxs_mtp_rowtop_draft2_trace_tg16.json`, `/tmp/dsv4_iq2xxs_mtp_rowtop_draft2_benchy_pp2048_tg32_runs3.json`, `/tmp/dsv4_iq2xxs_mtp_rowtop_regression.out`, `/tmp/dsv4_iq2xxs_mtp_rowtop_regression_retry.out`.
