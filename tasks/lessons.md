...
- **AMD APU UMA mode is required for large-model offload on Strix Halo / unified-memory iGPUs.** First HIP test of DSv4-Flash IQ2_XS with `-ngl 99` on Strix Halo silently stalled at 99% GPU memory allocation — the iGPU's logical "GPU memory pool" was filled before all layers were loaded. The model sits in system RAM (it's a unified memory architecture) but the HIP runtime treats the GPU pool as a separate region by default. Setting `GGML_CUDA_ENABLE_UNIFIED_MEMORY=1` switches the allocator to UMA mode and the load completes normally. The lesson: when an iGPU/APU silently stalls during model load, check `rocm-smi --showmemuse` for VRAM% — 99% on partial load means UMA isn't enabled.
- **Use long prompts (>=256 tokens) for prompt-eval benchmarks.** A 25-token short prompt has so much per-batch overhead (graph reuse, mctx setup, scheduler reservation) that the prefill TPS reading is meaningless. Long-prompt prefill is what reflects actual model throughput. For DSv4-Flash IQ2_XS on a 438-token prompt: CUDA / 3-GPU full offload = 413 t/s prefill, 38 t/s decode. HIP / Strix Halo iGPU full offload (UMA) = 77 t/s prefill, 12 t/s decode. The ~5x prefill / ~3x decode gap is better than the ~12x raw-FLOPs hardware ratio would predict — the kernels are doing fine, the iGPU just has less compute.
- **Never delete existing GGUFs without explicit user permission.** Disk-full situations during quant feel like an emergency but are not. The right move is to stop, report the disk situation, and ask which files the user wants to remove. Existing GGUFs are irreplaceable artifacts (re-quanting takes hours + GCP spend); a partial failed output is the only safe delete.
- **GCP imatrix jobs for custom-arch models MUST pass `--llama-repo` + `--llama-branch`.** The VM startup script builds upstream llama.cpp by default; any model with a custom architecture (`deepseek4`, etc.) that doesn't exist in upstream will fail immediately with `unknown model architecture`. Always pass `--llama-repo <fork> --llama-branch <branch>` when the architecture is not yet merged upstream.
- **Single-iteration A/B perf comparisons lie.** During the SWIGLU_CLAMPED fusion-args work, a single back-to-back run showed prefill at 401 t/s (current) vs 413 t/s (baseline) and looked like a 2.9% regression. After running 3 iterations at each of 3 prompt sizes (~100, 438, 1300 tokens), the actual numbers were prefill -1.6% / +0.4% / +0.13% (within noise on all sizes; net flat) and decode +2.4% / +1.5% / +4.4% (consistent positive). The change was a clean win all along; the original 401 was a one-shot unlucky tail. **Always run >=3 iterations at multiple prompt sizes before declaring a regression** — prefill especially has high run-to-run variance because the GPU clock state, memory pressure, and CUDA graph cache state on first invocation after a binary rebuild all amplify outliers.
- **Always implement `can_reuse()` on every custom `llm_graph_input_i` subclass.** The base-class default returns `false`. If even one input in the graph returns false, the framework rebuilds the entire compute graph on every step, which silently disables CUDA graph capture / replay and the per-step launch-overhead reduction it provides. DSv4 had four custom input classes (static i32, prompt window mask, prompt comp mask, abs comp mask) that all inherited the default. Implementing trivial shape-equality `can_reuse()` overrides on each one took the graph reuse rate from 0 to 197/200 (98%) and decode TPS from 38.6 t/s to 49.5 t/s on IQ2_XS full GPU offload — a 28% improvement from a few lines of bookkeeping. **Always check `graphs reused = N` in the perf-print summary; if it is 0 you have free perf on the table.**
...
- Treat prompt-cache/checkpoint restore as a target-state discontinuity for any private speculative sidecar state. If exact sidecar-state restoration is not implemented, clear private MTP raw/HC state at restore/truncation boundaries.
- Do not bypass an entire workload class (for example tool calls) to hide speculative verifier bugs. If target-only generation has special sampling state, the speculative verifier must mirror that state with a local copy while sampling/accepting draft rows.
