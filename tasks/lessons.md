...
- **AMD APU UMA mode is required for large-model offload on Strix Halo / unified-memory iGPUs.** First HIP test of DSv4-Flash IQ2_XS with `-ngl 99` on Strix Halo silently stalled at 99% GPU memory allocation — the iGPU's logical "GPU memory pool" was filled before all layers were loaded. The model sits in system RAM (it's a unified memory architecture) but the HIP runtime treats the GPU pool as a separate region by default. Setting `GGML_CUDA_ENABLE_UNIFIED_MEMORY=1` switches the allocator to UMA mode and the load completes normally. Resulting decode TPS jumped from 8.57 t/s (partial-offload, 8/44 layers) to 13.3 t/s (full-offload, 44/44 layers). The lesson: when an iGPU/APU silently stalls during model load, check `rocm-smi --showmemuse` for VRAM% — 99% on partial load means UMA isn't enabled.
