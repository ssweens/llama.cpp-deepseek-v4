...
- Corrected safety scope after review: do not bypass tool-call workloads. The server clears private MTP state on prompt-cache load, checkpoint restore, and prompt truncation, and the speculative verifier mirrors DeepSeek4's greedy-in-tool-call sampling with a local parser-state copy while verifying draft rows. Exact verifier/frontier work is still needed for larger draft-depth wins.
