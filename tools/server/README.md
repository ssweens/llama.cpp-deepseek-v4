...
| `--spec-type [none\|mtp\|ngram-cache\|ngram-simple\|ngram-map-k\|ngram-map-k4v\|ngram-mod]` | type of speculative decoding to use when no draft model is provided (default: none). For model-native MTP, use `--spec-type mtp --model-draft <MTP-support.gguf>`; `--draft-max` controls the draft cap. (env: LLAMA_ARG_SPEC_TYPE) |
